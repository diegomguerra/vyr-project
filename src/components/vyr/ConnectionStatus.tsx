...
export default function ConnectionStatus({ connection, onTap }: ConnectionStatusProps) {
  if (!connection.connected || !connection.provider) {
    return (
      <button
        onClick={onTap}
        className="flex items-center gap-2 bg-vyr-status-negative/15 border border-vyr-status-negative/30 rounded-full px-3.5 py-1.5 transition-all active:opacity-80 animate-pulse-slow"
      >
        <div className="relative">
          <AlertCircle className="w-4 h-4 text-vyr-status-negative" />
          <span className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-vyr-status-negative rounded-full" />
        </div>
        <span className="text-vyr-status-negative text-xs font-semibold tracking-wide">
          Sem wearable
        </span>
      </button>
    );
