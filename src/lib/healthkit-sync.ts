...
type RingDailyDataInsert = Database["public"]["Tables"]["ring_daily_data"]["Insert"];

export async function connectAppleHealth(): Promise<SyncResult> {
  const available = await isHealthKitAvailable();
  if (!available) return { success: false, error: "HealthKit não disponível neste dispositivo." };

  const authorized = await requestHealthKitPermissions();
  if (!authorized) return { success: false, error: "Permissões do Apple Health não foram concedidas." };
...
export async function syncHealthKitData(): Promise<SyncResult> {
  ...
  if (!authorized) return { success: false, error: "Permissões Apple Health revogadas." };

  const today = new Date().toISOString().slice(0, 10);
  const data = await readHealthKitData(today);

  if (!data) return { success: true, metricsWritten: false };

const metrics: Database["public"]["Tables"]["ring_daily_data"]["Insert"]["metrics"] = {};
  ...
  try {
    const upsertPayload: RingDailyDataInsert = {
      user_id: userId,
      day: today,
    ...
    if (error) {
      ...
      if (error.code === "42P10") {
        const insertPayload: RingDailyDataInsert = {
          user_id: userId,
          day: today,
          source_provider: "apple_health",
          metrics,
        ...
        const { error: insertError } = await supabase.from("ring_daily_data").insert(insertPayload);
        ...
        if (insertError) {
          ...
          toast({
            title: "Falha ao sincronizar dados do Health",
...
export async function getAppleHealthStatus(
  ...
  if (!data || data.status !== "active") return { connected: false, lastSync: null };
...
async function upsertIntegration(userId: string, provider: WearableProvider, status: string) {
