...
-- Drop and recreate each policy explicitly as PERMISSIVE
...
-- referencias_populacionais (keep as-is, read-only public)
DROP POLICY IF EXISTS "Anyone can read referencias" ON public.referencias_populacionais;
CREATE POLICY "Anyone can read referencias" ON public.referencias_populacionais FOR SELECT USING (true);
...
-- webhook_logs (service role only - keep restrictive is fine since it targets service_role)
