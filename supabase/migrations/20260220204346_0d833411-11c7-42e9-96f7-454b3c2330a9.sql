...
-- RESTRICTIVE policies block access even when user_id = auth.uid() matches
...
CREATE POLICY ring_daily_data_select_own ON public.ring_daily_data
  AS PERMISSIVE FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY ring_daily_data_insert_own ON public.ring_daily_data
  AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
CREATE POLICY ring_daily_data_update_own ON public.ring_daily_data
  AS PERMISSIVE FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
CREATE POLICY ring_daily_data_delete_own ON public.ring_daily_data
...
CREATE POLICY computed_states_select_own ON public.computed_states
  AS PERMISSIVE FOR SELECT TO authenticated USING (user_id = auth.uid());
CREATE POLICY computed_states_insert_own ON public.computed_states
  AS PERMISSIVE FOR INSERT TO authenticated WITH CHECK (user_id = auth.uid());
CREATE POLICY computed_states_update_own ON public.computed_states
  AS PERMISSIVE FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
CREATE POLICY computed_states_delete_own ON public.computed_states
  AS PERMISSIVE FOR DELETE TO authenticated USING (user_id = auth.uid());
...
CREATE POLICY participantes_update_own ON public.participantes
  AS PERMISSIVE FOR UPDATE TO authenticated USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
CREATE POLICY participantes_delete_own ON public.participantes
  AS PERMISSIVE FOR DELETE TO authenticated USING (user_id = auth.uid());
